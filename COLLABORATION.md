...
- 共享/协议/：寄存器、数据帧、DMA、中断和 DSP 接口；
- 共享/版本/：接口版本、PL build ID 和兼容关系；
- 共享/测试/：PS 和 PL 共同使用的测试数据和验证说明；
...
Codex 不是常驻进程，不能自动唤醒另一边。新需求按以下方式交接：

1. 工程师建立 GitHub Issue，用于提醒另一位工程师；
2. 在 docs/协作记录/请求/ 建立唯一编号的请求文件；
3. 工程师启动自己的 Codex，要求它拉取 main 并读取请求文件；
4. 目标工程师收到 GitHub 通知后，启动自己的 Codex 再次拉取并处理；
5. 需要改变寄存器、数据帧、DMA、IRQ、DSP、错误或性能的内容，先由工程师裁定；
6. Codex 实现后提交并推送 main，在请求文件中填写提交号和验证结果。

GitHub Issue 负责提醒人，请求文件负责让 Codex 读取和追溯，共享/协议/ 负责保存正式接口结论，Git 提交负责保存代码状态。项目不使用 Pull Request。
...
共享/协议/、共享/版本/ 和共享/测试/ 属于双方共同使用的目录。修改共享文件前必须先查看请求记录；涉及接口的修改必须完成工程师裁定，并避免同时编辑同一个文件。

## 接口规则

涉及寄存器、数据帧、DMA、IRQ、DSP 时序、设备树、错误、复位、超时、性能或显控格式的内容，必须写入共享/协议/ 和必要的 docs/协作记录/。
